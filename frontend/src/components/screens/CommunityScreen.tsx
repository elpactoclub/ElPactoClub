...
const STATIC_POSTS: ApiPost[] = [
  {
    id: "s1", authorName: "Herson", authorAvatar: "H", authorRole: "creator", type: "text", isOnline: true,
    content: "Entrenamiento terminado 💪 Preparando el MVP'S TOUR 3x3. ¿Quién va a estar ahí el 1 de julio? Os espero 🏀 #ElPacto",
    reactions: { "🔥": 234, "🏀": 89, "👏": 56, "👑": 12 },
    likesCount: 342, commentsCount: 2,
    createdAt: new Date(Date.now() - 3600000).toISOString(),
  },
  {
    id: "s2", authorName: "Violeta Verano", authorAvatar: "V", authorRole: "creator", type: "poll",
    content: "¿Qué es más difícil de ejecutar en un partido? 🏀",
    pollOptions: ["El triple en momento clave", "El mate con contacto"],
    pollVotes: { "El triple en momento clave": 773, "El mate con contacto": 474 },
    reactions: { "🔥": 98, "🏀": 45 },
    likesCount: 218, commentsCount: 0,
    createdAt: new Date(Date.now() - 7200000).toISOString(),
  },
  {
    id: "s3", authorName: "Elvis Ude", authorAvatar: "E", authorRole: "creator", type: "challenge",
    content: "🏀 RETO SEMANAL: Graba tu mejor movimiento de 1vs1 y súbelo con #ElPactoReto — el mejor gana 200 créditos 🔥",
    reactions: { "🔥": 312, "🏀": 178, "👏": 89 },
    likesCount: 456, commentsCount: 0,
    createdAt: new Date(Date.now() - 18000000).toISOString(),
  },
  {
    id: "s4", authorName: "MikelFan23", authorAvatar: "M", authorRole: "socio", type: "text",
    content: "Primera charla de El Pacto y @Herson respondió mi pregunta 🙌 Esto es diferente a cualquier comunidad de basket.",
    reactions: {},
    likesCount: 89, commentsCount: 0,
...
function PostAvatar({ name, role, size = 44 }: { name: string; role: string; size?: number }) {
  ...
  const color = role === "creator"
    ? name === "Herson" ? "#22C55E" : name === "Violeta Verano" ? "#F472B6" : "#A78BFA"
    : "#888";
  if (photo) {
...
function FeedTab() {
  ...
  const { avatar, liked, toggleLike } = useUserStore();
  ...
  useEffect(() => {
    api.get("/community/posts")
      .then((r) => setPosts(r.data.length > 0 ? r.data : STATIC_POSTS))
  ...
  return (
    <div className="community-feed" style={{ display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ display: "flex", gap: 16, overflowX: "auto", padding: "14px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)" }} className="hide-scrollbar">
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, flexShrink: 0, cursor: "pointer" }} onClick={openPostModal}>
        ...
        {[
          ...
          { name: "MikelFan23", photo: null, hasNew: false, color: "#60A5FA" },
          { name: "BasketQueen", photo: null, hasNew: false, color: "#A78BFA" },
        ].map((s) => (
          <div key={s.name} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, flexShrink: 0, cursor: "pointer", position: "relative" }} onClick={() => showToast(`${s.name} story · próximamente`)}>
            <div style={{ padding: 3, borderRadius: "50%", background: s.hasNew ? "linear-gradient(135deg,#F0E040,#FF6B1A)" : "rgba(255,255,255,0.1)" }}>
              {s.photo ? (
                ...
                <div style={{ width: 54, height: 54, borderRadius: "50%", background: (s.color ?? "#555") + "22", border: "2px solid #111", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 700, color: s.color ?? "#aaa" }}>{s.name[0]}</div>
              )}
            </div>
            {s.hasNew && <div style={{ position: "absolute", top: 0, right: -2, background: "#F0E040", color: "#000", fontSize: 7, fontWeight: 900, padding: "2px 4px", borderRadius: 4, border: "2px solid #111", letterSpacing: 0.5 }}>NEW</div>}
      ...
      {/* Post input */}
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)", cursor: "pointer" }} onClick={openPostModal}>
        ...
        <div style={{ flex: 1, background: "#1a1a1a", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 20, padding: "9px 16px", fontSize: 13, color: "var(--color-muted)" }}>¿Qué está pasando en El Pacto?</div>
        <span style={{ fontSize: 18 }}>📸</span>
        <span style={{ fontSize: 18 }}>📊</span>
      </div>

      {loading && <div style={{ padding: "32px", textAlign: "center", fontSize: 13, color: "var(--color-muted)" }}>Cargando feed... ⚡</div>}
        ...
        return (
          <div key={post.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)", borderLeft: `3px solid ${isCreator ? "var(--color-accent)" : "transparent"}` }}>
            <div style={{ display: "flex", gap: 12, alignItems: "flex-start", padding: "16px 16px 10px" }}>
              ...
              <button onClick={() => showToast("Opciones")} style={{ background: "transparent", border: "none", color: "var(--color-muted)", cursor: "pointer", fontSize: 18, padding: "0 4px", lineHeight: 1 }}>···</button>
            </div>
            ...
            {post.type === "poll" && post.pollOptions && (
              <div style={{ margin: "0 16px 12px", borderRadius: 10, overflow: "hidden", border: "1px solid rgba(255,255,255,0.07)" }}>
                {post.pollOptions.map((opt) => {
                  ...
                  return (
                    <button key={opt} onClick={() => handlePollVote(post.id, opt)} style={{ width: "100%", padding: "13px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", background: "transparent", border: "none", borderBottom: "1px solid rgba(255,255,255,0.05)", cursor: "pointer", position: "relative", overflow: "hidden", textAlign: "left" }}>
                ...
                <div style={{ padding: "10px 16px", fontSize: 11, color: "var(--color-muted)", borderTop: "1px solid rgba(255,255,255,0.05)" }}>📊 {pollTotal.toLocaleString()} votos · Cierra en 18h</div>
              </div>
            )}
            ...
            {Object.keys(reactions).length > 0 && (
              <div style={{ display: "flex", gap: 8, padding: "0 16px 12px", flexWrap: "wrap" }}>
                {Object.entries(reactions).map(([emoji, count]) => (
                  <button key={emoji} onClick={() => showToast(`${emoji} +1`)} style={{ display: "flex", alignItems: "center", gap: 5, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 20, padding: "5px 10px", cursor: "pointer", fontSize: 13 }}>
                    <span>{emoji}</span>
            ...
            <div style={{ display: "flex", gap: 18, padding: "6px 16px 16px", alignItems: "center" }}>
              <button onClick={() => handleLike(post.id)} style={{ background: "transparent", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 5, fontSize: 13, color: "var(--color-muted)", fontFamily: "var(--font-body)" }}>
                {isLiked ? "❤️" : "🤍"} <span>{post.likesCount + (isLiked ? 1 : 0)}</span>
              </button>
              {(post.commentsCount ?? 0) > 0 && (
                <button onClick={() => showToast("Comentarios próximamente")} style={{ background: "transparent", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 5, fontSize: 13, color: "var(--color-muted)", fontFamily: "var(--font-body)" }}>💬 <span>{post.commentsCount}</span></button>
              )}
              <button onClick={() => showToast("¡Compartido! 📤")} style={{ background: "transparent", border: "none", cursor: "pointer", fontSize: 13, color: "var(--color-muted)", fontFamily: "var(--font-body)" }}>↗ Compartir</button>
            </div>
          </div>
        );
      })}
...
const STATIC_MESSAGES: ChatMsg[] = [
  { id: "cm1", content: "Buenos días familia 🏀 Esta semana es clave para el MVP'S TOUR 3x3. Necesito que todos votéis el quinteto.", authorName: "Herson", authorAvatar: "H", authorRole: "creator", createdAt: new Date(Date.now() - 7200000).toISOString() },
  { id: "cm2", content: "¡Q&A esta tarde a las 18h! Mandadme vuestras preguntas aquí ✨", authorName: "Violeta Verano", authorAvatar: "V", authorRole: "creator", createdAt: new Date(Date.now() - 5400000).toISOString() },
  { id: "cm3", content: "¿Alguien se apunta al sorteo? 🔥", authorName: "MikelFan23", authorAvatar: "M", authorRole: "socio", createdAt: new Date(Date.now() - 3600000).toISOString() },
  { id: "cm4", content: "Yo ya tengo 3 entradas jaja", authorName: "BasketQueen", authorAvatar: "B", authorRole: "fan", createdAt: new Date(Date.now() - 1800000).toISOString() },
];
...
function getAuthorColor(name: string | undefined, role: string) {
  ...
  const seed = name && name.length > 0 ? name.charCodeAt(0) : 0;
...
function ChatTab() {
  ...
  const [messages, setMessages] = useState<ChatMsg[]>(STATIC_MESSAGES);
  const [input, setInput] = useState("");
  const messagesEndRef = useRef<HTMLDivElement>(null);

  const fetchMessages = (channel: Channel) =>
    api.get(`/community/messages?channel=${channel}`)
      .then((r) => { if (r.data.length > 0) setMessages(r.data); })
      .catch(() => {});
  ...
  const sendChat = async (text: string) => {
    ...
    setInput("");
    const optimistic: ChatMsg = { id: Date.now().toString(), content: trimmed, authorName: myName, authorAvatar: myName[0] ?? "?", authorRole: "fan", createdAt: new Date().toISOString() };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
      ...
      {/* Stats */}
      <div style={{ padding: "8px 16px", fontSize: 11, color: "var(--color-green)", display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
        <span style={{ display: "inline-block", width: 7, height: 7, borderRadius: "50%", background: "var(--color-green)" }} />
        1,247 fans · +2 XP por mensaje
      </div>
          ...
          return (
            <div key={m.id || i} style={{ display: "flex", gap: 10, alignItems: "flex-start", padding: "10px 0" }}>
              ...
              <div style={{ width: 36, height: 36, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, background: color + "22", color, border: `1.5px solid ${color}33` }}>
                {m.authorAvatar || (m.authorName && m.authorName[0]) || "?"}
              </div>
              {/* Bubble */}
              <div style={{ flex: 1, background: "#1a1a1a", borderRadius: 10, padding: "10px 14px", ...(isCreator ? { borderLeft: `2px solid ${color}` } : {}) }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
                  ...
                  {isCreator && (
                    <span style={{ fontSize: 7, fontWeight: 900, background: color + "22", color, padding: "2px 5px", borderRadius: 4 }}>CREADOR</span>
                  )}
                  {m.authorRole === "socio" && (
                    <span style={{ fontSize: 7, fontWeight: 900, background: "rgba(255,255,255,0.08)", color: "#888", padding: "2px 5px", borderRadius: 4 }}>SOCIO</span>
                  )}
                </div>
...
const STATIC_VOTES: VoteDecision[] = [
  { id: "v1", category: "PREGUNTA DEL DÍA", votationType: "pregunta", title: "¿Quién será el MVP del tour?", options: ["Elvis Ude 🏀", "Herson 🔥", "Violeta Verano ⭐"], creditsCost: 0, xpReward: 4, results: {} },
  { id: "v2", category: "CELEBRACIÓN", votationType: "votacion", title: "¿Qué celebración hacemos cuando metamos un triple?", options: ["Señal del Pacto ✋", "Salto colectivo 👡", "Apuntar a la cámara 📷"], creditsCost: 5, xpReward: 5, results: {} },
  { id: "v3", category: "DISEÑO", votationType: "votacion", title: "Ayúdanos a diseñar la segunda camiseta", options: ["Blanco con amarillo ⚡", "Negro con dorado 🔥", "Gris con relieve 👑"], creditsCost: 5, xpReward: 5, results: {} },
  { id: "v4", category: "CONTENIDO", votationType: "votacion", title: "¿Qué contenido subimos la próxima semana?", options: ["Behind the scenes 🎬", "Un día con Herson 🏀", "Mini documental 🎬"], creditsCost: 5, xpReward: 5, results: {} },
  { id: "v5", category: "APUESTA GRUPAL", votationType: "apuesta", title: "¿Ganamos la parada de Vilanova?", options: ["Sí ganamos 🏆", "No ganamos 😅"], creditsCost: 20, xpReward: 10, results: {}, betPool: 6840, participants: 342, closingDate: "28 Jun", betInfo: "Si +60% acierta, todos dobláis" },
  { id: "v6", category: "ENCUESTA SEMANAL", votationType: "encuesta", title: "¿Qué marca de zapatillas te gusta más?", options: ["Adidas", "Nike ⚡", "Puma", "WOW", "Rigorer"], creditsCost: 0, xpReward: 2, results: {} },
];
...
function VoteTab() {
  ...
  useEffect(() => {
    api.get("/gamification/votes")
      .then((r) => setVotes(r.data.length > 0 ? r.data : STATIC_VOTES))
      .catch(() => setVotes(STATIC_VOTES))
      .finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="community-vote" style={{ padding: "12px 12px 24px", display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <div style={{ background: "#1a1a1a", borderRadius: 10, padding: "18px 20px", border: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <div style={{ fontSize: 12, color: "var(--color-muted)" }}>Vota las decisiones del club</div>
      </div>
        ...
        return (
          <div
            ...
            style={{
              borderRadius: 10,
              overflow: "hidden",
              border: isApuesta ? "1px solid rgba(167,139,250,0.2)" : "1px solid rgba(255,255,255,0.06)",
              background: isApuesta ? "#0e0a18" : "#1a1a1a",
            }}
          >
            {/* Category header row */}
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      padding: "14px 16px",
                      borderRadius: 8,
                      border: isMyVote ? "1px solid rgba(240,224,64,0.4)" : isCorrect ? "1px solid rgba(34,197,94,0.4)" : "1px solid transparent",
                      background: isMyVote ? "rgba(240,224,64,0.07)" : isCorrect ? "rgba(34,197,94,0.07)" : "rgba(255,255,255,0.05)",
                      cursor: hasVoted || settled ? "default" : "pointer",
                      textAlign: "left",
                      position: "relative",
                      overflow: "hidden",
                  ...
                  >
                    {hasVoted && pct > 0 && (
                      <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, background: isMyVote ? "rgba(240,224,64,0.08)" : "rgba(255,255,255,0.03)", width: `${pct}%`, transition: "width 0.5s ease" }} />
                    )}
            ...
            {isApuesta && (
              <div style={{ padding: "10px 16px 14px", fontSize: 11, color: "var(--color-muted)", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                {v.participants} fans · Cierra {v.closingDate}
              </div>
            )}
          </div>
        );
...
export default function CommunityScreen() {
  ...
  return (
    <div className="community-screen" style={{ display: "flex", flexDirection: "column", height: isChat ? "100%" : "auto" }}>
      ...
      {/* Content — Chat gets flex:1 to fill remaining height */}
      {isChat ? (
        <ChatTab />
      ) : (
